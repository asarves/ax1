...
Ext.define('Ext.ux.calendar.editor.EventEditor', {
	...
	onDetailFn : function() {
	    this.obj.bindEl.bindEvent.subject = this.subjectField.getValue();
	    this.obj.bindEl.bindEvent.content = this.contentField.getValue();
		this.hideEditor();
	...
	onCalendarSelectFn : function(field, val, options) {
		...
		if (coverEl && !coverEl.hold) {
		    ...
		    this.bindEl.bindEvent.calendarId = value;
	...
	onSaveFn : function() {
	    if(this.bindEl.bindEvent.calendarId == -1) {
		Ext.Msg.show({
		    title:'Informations',
		    msg: 'Merci de choisir un agenda',
		    buttons: Ext.Msg.OK,
		    icon: Ext.Msg.ERROR
		});
		cview.fireEvent('canceldetail');
...
//			    console.log("event.finished <"+event.finished+">");
	...
	popup : function(obj) {
		...
		if ('add' == this.action) {
		    ...
		    // this.bindEl.bindEvent.calendarId = null;
		} else {

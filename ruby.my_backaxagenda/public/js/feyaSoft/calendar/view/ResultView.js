...
Ext.define('Ext.ux.calendar.view.ResultView', {
	...
	initComponent : function() {
		...
		var store = Ext.ux.calendar.Mask.getEventStore(
		    Ext.ux.calendar.CONST.searchURL, this.pageSize, this.matchText);
		var columns = [{
					header : '',
					sortable : true,
					menuDisabled : true,
	...
	onRowClickFn : function(grid, record, item, rowIndex, e) {
		...
		var event = {
		    ...
		    contact : rd.data.contact,
		    societe : rd.data.societe,
		    usertodo : rd.data.usertodo
		};
			...
			if (rowEl) {
				...
				var obj = {
					...
					onLayout : function(event) {
					    ...
					    rd.set('usertodo', event.usertodo);
						// store.commitChanges();
					},
					action : 'update'
				}
	...
	expireRenderFn : function(value, meta, rd, row, col, store) {
		...
		var endRow = Ext.ux.calendar.Mask.getRowFromHM(rd.data.endTime,
				this.intervalSlot);
	    var endTime = rd.data.endTime == '24:00' ? '00:00' : rd.data.endTime;
	    var str = rd.data.ymd + ' ' + endTime;

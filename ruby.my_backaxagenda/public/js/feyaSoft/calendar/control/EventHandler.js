        ...
        ['<div eid="{id}" name="x-event-{day}-{eday}-{id}" ',
            ...
            '<div style="height:18px;">',
                '<table width="100%" cellspacing="0" cellpadding="0" border="0"><tbody><tr>',
                    '<td style="{left-style}">',
                        '<div class="x-calendar-{color}-event-ltcorner-clear  x-event-lt-default" style="{lt-corner-style}">&nbsp;</div>',
                    '</td>',
                    '<td>',
                    ...
                    '<td style="{right-style}">',
                        '<div class="x-calendar-{color}-event-rtcorner-clear  x-event-rt-default" style="{rt-corner-style}">&nbsp;</div>',
                    '</td>',
                '</tr></tbody></table>',
            '</div>',
...
Ext.extend(Ext.ux.calendar.EventHandler, Ext.util.Observable, {
    ...
    onLoadUserEvents: function(usertodoValues) {
        // this.mainPanel.calendarContainer.currentView.fireEvent('beforeremoteload');
        // this.mainPanel.calendarContainer.currentView.cleanup();
	
	var curView = this.mainPanel.calendarContainer.currentView;
	curView.fireEvent('beforeremoteload');
    	var vDate=Ext.Date;
	var startDate = curView.daySet[0];
	var endDate = curView.daySet[curView.daySet.length-1];

	if(1 == curView.daySet.length){
            endDate =  vDate.add(endDate,Ext.Date.DAY, 1);
        }


	console.log("LoadEvent from EventHandler");
	Ext.usertodoValues = usertodoValues;
	this.ds.loadEvent(startDate, endDate, usertodoValues, function(eventSet) {
//	    console.log("onLoadUserEvents success");
            var cc = this.mainPanel.calendarContainer;
            var cview = cc.currentView;
	    cview.fireEvent('beforeremoteload');
	    cview.cleanup();

	    var glayout = this.calendarLayout;
	    var wlist = eventSet['whole'];
	    // glayout.wholeList['whole'] = [];
	    glayout.updateWholeList(wlist, 'replace');
	    cview.showEvents(eventSet, true);
	    cview.fireEvent('remoteload');
            // this.calendarLayout.resetLayout({
            //     hideCalendar:true,
    ...
    onUsersFilterFn: function(usertodoValue) { 
...
//	console.log("eh.onSelectUsersFn: usertodoValue <"+usertodoValue+">");
	...
	// var wl = this.calendarLayout.getWholeList(cview, null);
	for(var p in this.calendarLayout.layoutSet) {
//	    console.log("whole list length <"+this.calendarLayout.layoutSet[p].getAllEvents());
	}
	// this.ds.showOnlyUsers(useraskedValue, usertodoValue, function(backObj) {
//	//     console.log("into the Fn");
//	//     console.log("total events <"+backObj.total+">");
	//     var events = backObj.results;
	//     for(var i = 0; i < events.length; i++) { 
	// 	// var ele = Ext.dom.Query.selectNode('div[eid='+events[i].id+']');
	// 	// dom_ele = new Ext.dom.Element(ele);
	// 	// dom_ele.setStyle("display", "none");
//	// 	console.log("id event <"+events[i].id+">");
	//     }
        //     this.calendarLayout.resetLayout({
        //         hideCalendar:true,
        //         deleteCalendar:false,
    ...
    generateInfo:function(event){
        ...
        if(0 == startRow && this.rowCount == endRow){
            ...
            hm = event.day+' '+Ext.ux.calendar.Mask.getIntervalFromRow(this.intervalSlot, startRow, this.hourFormat)+' à ';
            if(event.day != event.eday){
                hm += event.eday+' ';
            }
    ...
    generateTitle:function(event){
	    ...
	    ev_societe = event.societe || event.subject;
    ...
    renderEvent:function(cview, rs, col, full){
	    ...
	    var do_not_render = false;

	    //// #######
	    Ext.usertodoValues != undefined && Ext.usertodoValues.forEach(
		function(element, index) {
		    user_id = element.split(/#/)[0];
		    calendar_abbr = element.split(/#/)[1];
		    console.log('user_id <'+user_id+'> calendar_abbr <'+calendar_abbr+'>');
		    if(calendar_abbr == 'je' && e.calendarId == 1 && e.usertodo == user_id) { 
			console.log('user in the filter 1 : return ');
			do_not_render = true;

			
		    }
		    if(calendar_abbr == 'jd' && e.calendarId == 2 && e.usertodo == user_id) {
			console.log('user in the filter 2 : return ');
			do_not_render = true;
		    }
		    if(do_not_render) {
			return;
	    ...
	    // alert('<' + e.usertodo + '> <' + e.calendarId + '>'); //  <' + Ext.usertodoValues + '>');
	    do_not_render = false;
		...
		var ltCornerStyle = '', rtCornerStyle = '';
                // #5B5B5B"; 
		if(e.tlj) { contentStyle += ";border: 4px solid black"; }
		else { 
		    contentStyle += ";border: 4px solid #FFB800"; 
		}
		// else { leftStyle += ";border: 4px solid red"; 
		//        rightStyle += ";border: 4px solid green"; 
		//        ltCornerStyle = "border: 4px solid red"; 
		//        titleStyle += "; border: 1px solid red; border-color: red;";
		//        rtCornerStyle = "border: 4px solid red"; }

	            var html = this.eventTpl.apply({
			'rt-corner-style': rtCornerStyle,
			'lt-corner-style': ltCornerStyle,
	                'id':e.eventId,
...
//	    console.log("Eventid <"+event.eventId+">");
    ...
    createRepeatEvent:function(event, cview, col){ 
        ...
        this.ds.createEvent(event, null, null, function(backObj){
	...
	if(typeof eEl.bindEvent.eventId === "string") { // new event
	    eEl.bindEvent.calendarId = 2;
	}
        this.editor.popup({
            bindEl:eEl,
            cview:cview,
	...
	// this.ds.updateEvent(event, oevent, function(backObj){
            var gLayout = this.calendarLayout;
            if(backObj.id){
                event.eventId = backObj.id;
            }
    ...
    loadRepeatEvent:function(fn, scope){
        // this.ds.loadRepeatEvent(undefined,undefined,undefined,function(eventSet){            
